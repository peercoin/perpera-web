  ...
  public async handleFileUri(e: any) {
    ...
    try {
      ...
      if (fileBuffer === 0) {
        ...
        ObservableHelper.fire('onNewFileHash', {fileBuffer, fileName: this.state.uri});
  ...
  public render() {
    return (
      <div className="AddComp">
        ...
        <div className="add-btn" title="input file">+<input className="file" type="file" onChange={this.handleFile}/></div>
        <div className="add-btn add-uri" title="input uri">+<input className="file" type="button" onClick={this.changeFileType}/></div>
        {!this.state.fileType && (
